...
export const EXPANDED_QUESTION_BANK: GMCQuestion[] = [
  // CARDIOVASCULAR MEDICINE - GMC MLA Compliant Questions
  {
    ...
    correctAnswer: 0,
    explanation: "This patient presents with inferior STEMI (ST elevation in leads II, III, aVF) requiring immediate evidence-based management according to current guidelines. ESC STEMI guidelines 2023 and NICE CG167 recommend dual antiplatelet therapy as first-line treatment: aspirin 300mg loading dose (chewed for rapid absorption) plus clopidogrel 600mg loading dose. Primary percutaneous coronary intervention (PCI) is the gold standard reperfusion strategy when available within 120 minutes of first medical contact, significantly superior to thrombolysis in reducing mortality and reinfarction rates. The patient's diabetes mellitus increases cardiovascular risk, making urgent intervention crucial. Aspirin irreversibly inhibits cyclooxygenase-1, reducing platelet aggregation, while clopidogrel blocks P2Y12 receptors providing additional antiplatelet effect. Primary PCI achieves TIMI-3 flow in >95% of cases compared to 60% with thrombolysis. Door-to-balloon time should be <90 minutes. Secondary prevention includes continuing dual antiplatelet therapy for 12 months, high-intensity statin therapy (atorvastatin 80mg), and ACE inhibitor if no contraindications. Blood pressure management and diabetic control are essential for long-term outcomes. This evidence-based approach follows international guidelines and reduces 30-day mortality from 10% to <5% when implemented correctly.",
    learningObjectives: [
    ...
    references: [
      ...
      "ESC Guidelines for STEMI management 2023",
...

  {
    ...
    correctAnswer: 1,
    explanation: "This patient presents with heart failure with reduced ejection fraction (HFrEF, EF 35%) following myocardial infarction. NICE CG108 and ESC Heart Failure Guidelines 2021 establish evidence-based treatment protocols for HFrEF. ACE inhibitors are first-line therapy, with ramipril started at 1.25mg daily and titrated to maximum tolerated dose (10mg daily) over 2-4 weeks, monitoring renal function and potassium levels. Multiple randomized controlled trials (SAVE, AIRE, TRACE) demonstrate 15-25% mortality reduction. Beta-blockers provide additional mortality benefit and should be introduced once patient stabilized on ACE inhibitor. Bisoprolol 1.25mg daily, titrated to 10mg daily, reduces mortality by 30% (CIBIS-II trial). The combination is synergistic, addressing neurohormonal activation in heart failure. BNP elevation (850 pg/ml) confirms diagnosis and correlates with prognosis. Loop diuretics like furosemide provide symptomatic relief from fluid overload but lack mortality benefit. Spironolactone may be added if symptoms persist despite optimal ACE inhibitor and beta-blocker therapy (RALES trial showed 30% mortality reduction). Device therapy (CRT/ICD) considered if EF remains ≤35% despite 3 months optimal medical therapy. Regular monitoring includes renal function, electrolytes, and symptom assessment. This evidence-based approach follows international guidelines and significantly improves both symptoms and survival in HFrEF patients.",
    learningObjectives: [
  ...
  // RESPIRATORY MEDICINE - GMC MLA Compliant Questions
  {
    ...
    correctAnswer: 1,
    explanation: "This case demonstrates severe community-acquired pneumonia requiring systematic assessment using CURB-65 severity scoring. CURB-65 components: Confusion (1 point - altered mental state reported), Urea elevation (1 point - likely given diabetes and age), Respiratory rate ≥30/min (0 points - currently 28/min but concerning trend), Blood pressure <90mmHg systolic (1 point - hypotensive at 100/65mmHg), age ≥65 years (1 point). Total score 3-4 indicates severe pneumonia. BTS Guidelines for Community Acquired Pneumonia 2015 and NICE CG191 recommend hospital admission for CURB-65 ≥2, with ITU consideration for score ≥3. This patient requires immediate IV antibiotics: first-line co-amoxiclav 1.2g TDS covers typical and atypical pathogens, or ceftriaxone 2g daily if penicillin allergic. Oxygen saturation 88% indicates type 1 respiratory failure requiring supplemental oxygen targeting 94-98%. The combination of hemodynamic instability, hypoxia, and high CURB-65 score warrants ITU assessment for possible organ support. Blood cultures, pneumococcal/legionella urinary antigens, and arterial blood gas analysis should be obtained. COPD and diabetes are additional risk factors for severe pneumonia and complications. Early appropriate antibiotic therapy within 4 hours improves outcomes. This evidence-based approach following BTS/NICE guidelines ensures optimal management of severe pneumonia with consideration for intensive care when indicated.",
    learningObjectives: [
    ...
    references: [
      "BTS Guidelines for Community Acquired Pneumonia 2015",
...

  {
    ...
    correctAnswer: 2,
    explanation: "This patient presents with acute COPD exacerbation meeting Anthonisen criteria for antibiotic therapy: increased breathlessness, increased sputum volume, and purulent sputum change. NICE CG101 COPD guidelines and GOLD 2023 recommendations provide evidence-based management protocols. Prednisolone 30mg daily for 5 days reduces inflammation, shortens recovery time, and improves lung function - supported by multiple RCTs showing significant benefit. Antibiotic therapy is indicated due to purulent sputum; amoxicillin 500mg TDS for 5 days covers common pathogens (Haemophilus influenzae, Streptococcus pneumoniae). Alternative antibiotics include doxycycline or clarithromycin if penicillin allergic. The dual approach addresses both inflammatory (prednisolone) and infective (antibiotic) components. Nebulized bronchodilators (salbutamol 2.5-5mg QDS, ipratropium 500mcg QDS) provide additional bronchodilation and symptom relief. Oxygen therapy should target 88-92% saturation to avoid CO2 retention. Assessment for non-invasive ventilation warranted if pH <7.35 or CO2 >6kPa. Smoking cessation counselling and review of inhaler technique essential. Pulmonary rehabilitation referral improves exercise tolerance and quality of life. This evidence-based approach following NICE guidelines reduces hospital length of stay, prevents treatment failure, and improves patient outcomes. Regular review ensures appropriate response to therapy and identifies patients requiring escalation of care.",
    learningObjectives: [
